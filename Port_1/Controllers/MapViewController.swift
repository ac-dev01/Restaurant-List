...
extension MapViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if let customAnnotaionView = mapView.dequeueReusableAnnotationView(withIdentifier: identifier) {
            annotationView = customAnnotaionView
        ...
        if let image = UIImage(named: "custom-annotation") {
            ...
            annotationView.centerOffset = CGPoint(x: -image.size.width / 2,
...
private extension MapViewController{
    ...
    func setupMap(_ annotations: [RestaurantItem]) {
        ...
        mapView.setRegion(manager.initialRegion(latDelta: 0.5,
                                                longDelta: 0.5),
                          animated: true)
